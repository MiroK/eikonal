...
std::vector<double>
polygon_generate(const size_t n, const std::vector<double>& center,
                 const double radius);

// given vertices of polygon (flattened [v0_x, v0_y, v1_x, v1_y, ... ])
// compute vertices that are on boundary of polygon
// t > 2 specifies number of vertices per edge of polygon
std::vector<double>
polygon_boundary(const std::vector<double>& vertices, const size_t t = 100);
